...
```bash
docker run --rm -e VERIFICATION_MODE=offline -v "/path/to/.gnupg:/opt/gnupg" leplusorg/pgp-verify-jar org.leplus:ristretto:2.0.0 junit:junit:4.13.1
```
...
```bash
docker run --rm -v "/path/to/.gnupg:/opt/gnupg" leplusorg/pgp-verify-jar --verification-mode=offline org.leplus:ristretto:2.0.0 junit:junit:4.13.1
```
